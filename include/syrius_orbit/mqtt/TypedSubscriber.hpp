...
public:
  ...
  explicit TypedSubscriber(MqttSubscriber &subscriber) noexcept
      : subscriber_(subscriber) {}

  template <typename T>
  [[nodiscard]] bool Subscribe(const std::string &topic_filter, int qos,
                               Callback<T> callback) {
    ...
    auto wrapped_callback = [callback = std::move(callback)](
      ...
      if (msg.payloadlen < 0) {
        PLOGW
            << "TypedSubscriber received message with negative payload length.";
        return;
      }
      ...
      if (msg.payload != nullptr && msg.payloadlen > 0)
        meta.payload.assign(static_cast<const char *>(msg.payload),
    ...
    return subscriber_.Subscribe(topic_filter, qos,
